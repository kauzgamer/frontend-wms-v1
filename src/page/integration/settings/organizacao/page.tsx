...
import type { UpsertOrganizationInput } from "@/lib/types/organization";
...
export function OrganizacaoPage() {
  ...
  function update<K extends keyof OrganizacaoForm>(
    key: K,
  ...
  async function handleSave() {
    ...
    try {
      ...
      const cnpjLimpo = form.cnpj ? form.cnpj.replace(/\D/g, "") : "";

      // Construir payload - CNPJ só é enviado se tiver 14 dígitos
      const payload: UpsertOrganizationInput = {
        codigo: form.codigo,
        nome: form.nome,
        ativo: form.ativo,
        timezone: form.timezone || undefined,
        createPrincipalDeposit: !!form.createPrincipalDeposit,
        ...(cnpjLimpo.length === 14 ? { cnpj: cnpjLimpo } : {}),
      };
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1 max-w-xl">
            Configurações base da organização utilizadas na integração e
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            className="h-9 bg-[#0c9abe] hover:bg-[#0a869d] text-white"
          >
            <Save className="size-4 mr-1" />{" "}
            {saving ? "Salvando..." : isLoading ? "Carregando..." : "Salvar"}
          </Button>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                maxLength={14}
              />
              {form.cnpj && form.cnpj.length > 0 && form.cnpj.length !== 14 && (
            ...
            <div className="flex flex-col gap-2 sm:col-span-2">
              ...
              <label className="inline-flex items-center gap-2 text-xs font-medium">
                <input
                  type="checkbox"
                  checked={!!form.createPrincipalDeposit}
                  onChange={(e) =>
                    update("createPrincipalDeposit", e.target.checked)
                  }
                  className="size-4 rounded border"
                />{" "}
                Criar/garantir Depósito PRINCIPAL ao salvar
              </label>
            </div>
          </div>
        </Card>
        <Card className="p-5 flex flex-col gap-5">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Campos informativos sincronizados
            </p>
          </div>
