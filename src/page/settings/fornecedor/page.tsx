    ...
    <div className="p-6 space-y-4">
      <Breadcrumb className="mb-2">
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage>Cadastro fornecedor</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>

      <h1 className="text-2xl font-semibold">Fornecedor</h1>

      <div className="border rounded-md p-3">
        <div className="text-sm font-medium mb-2">Fornecedores cadastrados</div>
        {status !== 'TODOS' && (
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <div className="relative">
              <label htmlFor="supplier-search" className="sr-only">Pesquisar</label>
              <input id="supplier-search" name="q" className="border rounded pl-8 pr-2 py-1 text-sm w-64" placeholder="Pesquisar por nome ou documento" value={q} onChange={(e) => setQ(e.target.value)} />
      ...
      <div className="flex gap-3 items-end">
        <div className="flex flex-col">
          <label htmlFor="supplier-status" className="text-sm">Situação</label>
          <select id="supplier-status" name="status" className="border rounded px-2 py-1" value={status} onChange={(e) => { setStatus(e.target.value as 'ATIVO' | 'INATIVO' | 'TODOS'); setPage(1) }}>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 text-sm">
            <label htmlFor="supplier-pageSize" className="text-gray-600">Exibir</label>
            <select id="supplier-pageSize" name="pageSize" className="border rounded px-2 py-1" value={pageSize} onChange={(e)=>{ const v=parseInt(e.target.value,10); setPageSize(v); setPage(1) }}>
...
function Row({ sup, onChanged }: { sup: Supplier; onChanged: () => void }) {
  ...
  const toggleAtivo = async () => {
    try {
      await updateMut.mutateAsync({ id: sup.id, data: { active: sup.situacao !== 'ATIVO' } })
  ...
  const onDelete = async () => {
    try {
      await deleteMut.mutateAsync(sup.id)
      toastApi.show({ message: 'Fornecedor excluído', kind: 'success' })
      onChanged()
    } catch (e) {
      ...
      <td className="px-3 py-2">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start">
            <DropdownMenuItem onClick={toggleAtivo}>{sup.situacao === 'ATIVO' ? 'Inativar' : 'Ativar'}</DropdownMenuItem>
        ...
        <ConfirmationDialog open={confirmOpen} title="Confirmação" description={`Deseja excluir o fornecedor “${sup.nome}”?`} confirmText="Excluir" onConfirm={onDelete} onCancel={() => setConfirmOpen(false)} />
      </td>
      <td className="px-3 py-2">{sup.documento}</td>
      <td className="px-3 py-2">{sup.nome}</td>
      <td className="px-3 py-2">{sup.uf}</td>
      <td className="px-3 py-2">{sup.inscricaoEstadual ?? 'Não informada'}</td>
      <td className="px-3 py-2"><span className={`px-2 py-1 rounded text-xs ${sup.situacao === 'ATIVO' ? 'bg-green-100 text-green-800' : 'bg-gray-200 text-gray-700'}`}>{sup.situacao}</span></td>
