...
const MOCK_DATA: DepositoItem[] = [
  { id: '1', descricao: 'PRINCIPAL', situacao: 'ATIVO', principal: true },
]
...
export function DepositoPage() {
  ...
  useEffect(() => {
    function handleClick(e: MouseEvent) {
      ...
      if (openId && !menuRef.current.contains(e.target as Node)) setOpenId(null)
    }
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-6 pt-4">
      ...
      <h1 className="text-3xl font-semibold leading-tight" style={{ color: '#4a5c60' }}>Depósito</h1>
        ...
        <div className="flex items-center gap-4 px-5 py-3 border-b bg-white/60">
          ...
          <div className="text-[11px] flex items-center gap-2">
            <span className="inline-flex items-center gap-1 rounded-md border px-2 py-1 bg-white leading-none">
              <span className="font-medium" style={{color:'#555'}}>Situação igual a:</span> <span className="text-[#0c9abe] font-semibold">Ativo</span>
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="border-b bg-white">
                ...
                <th className="text-left font-semibold px-6 py-2 w-48" style={{ color: '#0c9abe' }}>Situação</th>
              </tr>
            </thead>
            <tbody>
              {itens.map((item, idx) => {
                  ...
                  <tr key={item.id} className={`border-b last:border-b-0 relative ${striped ? 'bg-[#f8f8f8]' : 'bg-white'} hover:bg-[#eef7fa]`}>
                    <td className="px-2 text-center align-middle relative border-r">
                      ...
                      {isOpen && (
                        ...
                        >
                          <button role="menuitem" className="w-full text-left px-3 py-2 text-sm hover:bg-muted/70" onClick={() => setOpenId(null)}>Detalhes</button>
                          <button role="menuitem" className="w-full text-left px-3 py-2 text-sm hover:bg-muted/70" onClick={() => setOpenId(null)}>Editar</button>
                          <button role="menuitem" className="w-full text-left px-3 py-2 text-sm hover:bg-muted/70 text-red-600" onClick={() => setOpenId(null)}>Inativar</button>
                        </div>
                      )}
                    </td>
                    <td className="px-6 py-2 font-semibold text-[12.5px] tracking-wide border-r" style={{color:'#4a5c60'}}>
                      <span className="inline-flex items-center gap-2">
                        {item.descricao}
                        {item.principal && (
                          <span className="text-[10px] font-semibold rounded-full bg-[#555] text-white px-2 py-[3px] leading-none">PRINCIPAL</span>
                        )}
                      </span>
                    </td>
                    <td className="px-6 py-2">
                      <div className="flex justify-end">
                        <span className="inline-flex items-center rounded-sm bg-[#009c80] text-white text-[11px] font-semibold px-4 h-7 leading-none justify-center min-w-32 tracking-wide" style={{letterSpacing:'0.5px'}}>{item.situacao}</span>
                      </div>
                    </td>
                  </tr>
                )
