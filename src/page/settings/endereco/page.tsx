...
export default function EnderecoPage() {
  ...
  const [advSituacoes, setAdvSituacoes] = useState<
    Array<"ATIVO" | "BLOQUEADO">
  >(["ATIVO", "BLOQUEADO"]);
  ...
  const estruturaOptions = useMemo(() => {
    return Array.from(
      new Set((addresses ?? []).map((a) => a.estruturaFisica))
    ).sort();
  }, [addresses]);
  ...
  const filteredAddresses = useMemo(() => {
    ...
    return addresses.filter((addr) => {
      ...
      if (
        advEnderecoCompleto &&
        !addr.enderecoCompleto
          .toLowerCase()
          .includes(advEnderecoCompleto.toLowerCase())
      ) {
        return false;
      }
      if (
        ...
        !addr.enderecoAbreviado
          .toLowerCase()
          .includes(advEnderecoAbreviado.toLowerCase())
      ) {
        return false;
      }
  ...
  const startResult =
    filteredAddresses.length > 0 ? (currentPage - 1) * pageSize + 1 : 0;
    ...
    <div className="flex flex-1 flex-col gap-6 p-6 pt-4 max-w-[calc(100vw-80px)]">
      ...
      <div className="w-full">
        ...
        <div className="mt-4 flex items-center justify-between gap-4">
          <h1
            className="text-3xl font-semibold leading-tight flex-shrink-0"
            style={{ color: "#4a5c60" }}
          >
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="flex flex-col gap-2">
                  ...
                  <Input
                    ...
                    value={advEnderecoCompleto}
                    onChange={(e) => setAdvEnderecoCompleto(e.target.value)}
                  />
                </div>
                <div className="flex flex-col gap-2">
                  <label className="text-sm font-semibold">
                  ...
                  <Input
                    placeholder="Informe a descrição mobile"
                    value={advEnderecoAbreviado}
                    onChange={(e) => setAdvEnderecoAbreviado(e.target.value)}
                  />
                </div>
                      ...
                      {(
                        ...
                        >
                          <input
                            type="checkbox"
                            readOnly
                            checked={advFuncoes.includes(fn)}
                            className="mr-2"
                          />{" "}
                          {fn}
                        </DropdownMenuItem>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <button className="w-full flex items-center justify-between rounded-md border px-3 py-2 text-left text-sm">
                        <span className="truncate">
                          {advDeposito ?? "Selecione o depósito"}
                        </span>
                        <ChevronDown className="size-4 text-gray-500" />
                      </button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="start" className="w-[320px]">
                      <DropdownMenuItem onClick={() => setAdvDeposito(null)}>
                        — Limpar —
                      </DropdownMenuItem>
                      {depositoOptions.map((opt) => (
                        <DropdownMenuItem
                          key={opt}
                          onClick={() => setAdvDeposito(opt)}
                        >
                          {opt}
                        </DropdownMenuItem>
                      ))}
                  ...
                  <div className="text-xs text-muted-foreground flex items-center gap-2">
                    <Info className="size-4" /> Escolha o depósito e a estrutura
                    física
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <button className="w-full flex items-center justify-between rounded-md border px-3 py-2 text-left text-sm">
                        <span className="truncate">
                          {advEstrutura ?? "Selecione a estrutura física"}
                        </span>
                        <ChevronDown className="size-4 text-gray-500" />
                      </button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent
                      align="start"
                      className="w-[320px] max-h-64 overflow-auto"
                    >
                      <DropdownMenuItem onClick={() => setAdvEstrutura(null)}>
                        — Limpar —
                      </DropdownMenuItem>
                      {estruturaOptions.map((opt) => (
                        <DropdownMenuItem
                          key={opt}
                          onClick={() => setAdvEstrutura(opt)}
                        >
                          {opt}
                        </DropdownMenuItem>
                      ))}
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex items-center gap-6">
                      <label className="inline-flex items-center gap-2 text-sm">
                        <input
                          type="checkbox"
                          checked={advAcessivel === "SIM"}
                          onChange={(e) =>
                            setAdvAcessivel(e.target.checked ? "SIM" : "NA")
                          }
                        />{" "}
                        Acessível a mão
                      </label>
                      <label className="inline-flex items-center gap-2 text-sm">
                        <input
                          type="checkbox"
                          checked={advAcessivel === "NAO"}
                          onChange={(e) =>
                            setAdvAcessivel(e.target.checked ? "NAO" : "NA")
                          }
                        />{" "}
                        Não acessível a mão
                      </label>
                      <label className="inline-flex items-center gap-2 text-sm">
                        <input
                          type="checkbox"
                          checked={advAcessivel === "NA"}
                          onChange={() => setAdvAcessivel("NA")}
                        />{" "}
                        Não aplicavel
                      </label>
                    </div>
              ...
              <div className="px-6 py-4 border-t flex items-center justify-end gap-3 bg-gray-50">
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    const setVal =
                      advSituacoes.includes("ATIVO") &&
                      advSituacoes.includes("BLOQUEADO")
                        ? "AB"
                        : advSituacoes.includes("ATIVO")
                        ? "ATIVO"
                        : advSituacoes.includes("BLOQUEADO")
          ...
          <Dialog.Portal>
            ...
            <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-[860px] max-w-[95vw] max-h-[80vh] overflow-auto rounded-md bg-white shadow-xl border">
              <div className="px-6 py-5 border-b">
                ...
                <Dialog.Description className="sr-only">
                  Selecione as colunas que deseja exibir na tabela de endereços.
                </Dialog.Description>
              </div>
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4 text-[15px]">
                {[
                  { key: "deposito", label: "Depósito" },
                  {
                    ...
                    label: "Endereço completo (Desktop)",
                  },
                  {
                    key: "enderecoAbreviado",
                    label: "Endereço abreviado (Dispositivos móveis)",
                  },
                  ...
                  {
                    key: "ocUnitizadorPrev",
                    label: "Ocupação unitizador prevista",
                  },
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Os endpoints de endereços ainda não foram implementados no
              backend.
            </p>
              ...
              <table className="w-full text-[13px]">
                <thead>
                  <tr className="border-b bg-white">
                    ...
                    {visibleCols.capUnitizador && (
                      <th className="text-left p-3 font-medium text-xs whitespace-nowrap min-w-[160px]">
                        Cap. Unitizador
                      </th>
                    )}
                    {visibleCols.capAltura && (
                      <th className="text-left p-3 font-medium text-xs whitespace-nowrap min-w-[120px]">
                        Cap. Altura
                      </th>
                    )}
                    {visibleCols.capComprimento && (
                      <th className="text-left p-3 font-medium text-xs whitespace-nowrap min-w-[150px]">
                        Cap. Comprimento
                      </th>
                    )}
                    {visibleCols.capLargura && (
                      <th className="text-left p-3 font-medium text-xs whitespace-nowrap min-w-[120px]">
                        Cap. Largura
                      </th>
                    )}
                    ...
                    {visibleCols.ocCubagem && (
                      <th className="text-left p-3 font-medium text-xs whitespace-nowrap min-w-[150px]">
                        Ocup. Cubagem
                      </th>
                    )}
                    {visibleCols.ocCubagemPrev && (
                      <th className="text-left p-3 font-medium text-xs whitespace-nowrap min-w-[180px]">
                        Ocup. Cubagem Prev.
                      </th>
                    )}
                    {visibleCols.ocPeso && (
                      <th className="text-left p-3 font-medium text-xs whitespace-nowrap min-w-[140px]">
                        Ocup. Peso
                      </th>
                    )}
                    {visibleCols.ocPesoPrev && (
                      <th className="text-left p-3 font-medium text-xs whitespace-nowrap min-w-[160px]">
                        Ocup. Peso Prev.
                      </th>
                    )}
                    {visibleCols.ocUnitizador && (
                      <th className="text-left p-3 font-medium text-xs whitespace-nowrap min-w-[180px]">
                        Ocup. Unitizador
                      </th>
                    )}
                    {visibleCols.ocUnitizadorPrev && (
                      <th className="text-left p-3 font-medium text-xs whitespace-nowrap min-w-[200px]">
                        Ocup. Unitizador Prev.
                      </th>
                    )}
                    {visibleCols.codigoBarras && (
                      <th className="text-left p-3 font-medium text-xs whitespace-nowrap min-w-[160px]">
                        Código de barras
                      </th>
                    )}
                    {visibleCols.situacao && (
                      <th className="text-left p-3 font-medium text-xs whitespace-nowrap min-w-[100px]">
                        Situação
                      </th>
                    )}
                    {visibleCols.motivoBloqueio && (
                      <th className="text-left p-3 font-medium text-xs whitespace-nowrap min-w-[180px]">
                        Motivo bloqueio
                      </th>
                    )}
                  ...
                  {paginatedAddresses.map((addr) => {
                    ...
                    return (
                      <tr
                        ...
                        onClick={() =>
                          navigate(`/settings/endereco/${addr.id}`)
                        }
                      >
                          ...
                          <DropdownMenu>
                            ...
                            >
                              ...
                              <DropdownMenuItem
                                onClick={() =>
                                  navigate(
                                    `/settings/endereco/${addr.id}?edit=1`
                                  )
                                }
                              >
                        ...
                        {visibleCols.capAltura && (
                          <td className="p-3 text-sm whitespace-nowrap">
                            {det.altura != null ? `${det.altura}cm` : "-"}
                          </td>
                        )}
                        {visibleCols.capComprimento && (
                          <td className="p-3 text-sm whitespace-nowrap">
                            {det.comprimento != null
                              ? `${det.comprimento}cm`
                              : "-"}
                          </td>
                        )}
                        {visibleCols.capLargura && (
                          <td className="p-3 text-sm whitespace-nowrap">
                            {det.largura != null ? `${det.largura}cm` : "-"}
                          </td>
                        )}
                        {visibleCols.capPeso && (
                          <td className="p-3 text-sm whitespace-nowrap">
                            {det.peso != null ? `${det.peso}kg` : "-"}
                          </td>
                        )}
                        {visibleCols.ocCubagem && (
                          <td className="p-3 text-sm whitespace-nowrap text-right">
                            -
                          </td>
                        )}
                        {visibleCols.ocCubagemPrev && (
            ...
            <div className="border-t p-3 bg-white">
              <div className="flex items-center justify-between text-xs text-gray-600 mb-2">
                <span className="font-medium">
                  {startResult} - {endResult} DE {filteredAddresses.length}{" "}
                  RESULTADOS
                </span>
                <div className="flex items-center gap-4">
                  {currentPage < totalPages && (
                    ...
                    >
                      Carregar mais{" "}
                      {Math.min(pageSize, filteredAddresses.length - endResult)}{" "}
                      resultados
                    </Button>
                  )}
