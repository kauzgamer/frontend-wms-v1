...
export function NewProductPage() {
  ...
  const { data: categories, isLoading: categoriesLoading } =
    useProductCategoriesForSelect();
  const { data: stockAttributes, isLoading: stockAttributesLoading } =
    useStockAttributes({ status: "ATIVO" });
  ...
  const [newSku, setNewSku] = useState<TempSku>({
    id: "",
    description: "",
    unitsPerSku: 1,
    fractional: false,
    generatePickingLabel: false,
    barcodes: [],
  ...
  const [activeTab, setActiveTab] = useState<"barcodes" | "dimensions">(
    "barcodes"
  );
  ...
  function confirmEditSku() {
    ...
    setSkus((prev) =>
      prev.map((s) => (s.id === editingSkuId ? { ...s, ...editDraft } : s))
    );
    setEditingSkuId(null);
    setEditDraft(null);
  }
  ...
  // Sheet helpers
  function openSheetForSku(
    id: string,
    tab: "barcodes" | "dimensions" = "barcodes"
  ) {
    ...
    setSkus((prev) =>
      prev.map((s) =>
        s.id === openSkuSheetId
          ? {
              ...s,
              barcodes: s.barcodes.includes(barcodeInput.trim())
                ? s.barcodes
                : [...s.barcodes, barcodeInput.trim()],
            }
          : s
      )
    );
  ...
  function removeBarcode(code: string) {
    setSkus((prev) =>
      prev.map((s) =>
        s.id === openSkuSheetId
          ? { ...s, barcodes: s.barcodes.filter((b) => b !== code) }
          : s
      )
    );
  }

  function updateDimensions<K extends keyof DimensionsDraft>(
    key: K,
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-6 pt-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <section>
          ...
          <div className="grid gap-6 grid-cols-1 md:grid-cols-3">
            <div className="flex flex-col gap-1">
              ...
              >
                <option value="">
                  {org ? "Selecionar unidade" : "Carregando..."}
                </option>
                {org && <option value={org.id}>{org.nome}</option>}
              </select>
            </div>
            <div className="flex flex-col gap-1 md:col-span-2">
              <label className="text-sm font-medium text-[#334b52]">
              ...
              <select
                multiple
                value={form.stockCharacteristics?.map(String) || []}
                onChange={(e) => {
                ...
                className="min-h-[120px] rounded border px-3 py-2 text-sm bg-white shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#0c9abe]"
              >
                {stockAttributesLoading && (
                  <option value="" disabled>
                    Carregando...
                  </option>
                )}
                {!stockAttributesLoading &&
                  (stockAttributes?.length ?? 0) === 0 && (
                    <option value="" disabled>
        ...
        <section>
          <h2 className="text-xs font-semibold tracking-wide text-[#008bb1] mb-3">
            SKU
          </h2>
          <div className="mb-3">
            <button
              ...
              onClick={startAddSku}
              disabled={addingSku}
              className="h-10 bg-[#008bb1] hover:bg-[#007697] text-white text-sm font-medium rounded px-4 disabled:opacity-50 inline-flex items-center gap-1"
            >
            ...
            <table className="w-full text-sm border-collapse">
              <thead className="bg-muted/40">
                <tr className="text-left">
                  ...
                  <th className="px-3 py-2 font-medium border-b border-r border-gray-200">
                    Descrição
                  </th>
                  <th className="px-3 py-2 font-medium border-b border-r border-gray-200">
                    Quantidade de unidades
                  </th>
                  <th className="px-3 py-2 font-medium text-center border-b border-r border-gray-200">
                    Fracionado
                  </th>
                  <th className="px-3 py-2 font-medium text-center border-b border-r border-gray-200">
              ...
              <tbody>
                    ...
                    <td className="px-2 py-2 border-b border-r border-gray-200">
                      <input
                        ...
                        onChange={(e) =>
                          setNewSku((s) => ({
                            ...s,
                            unitsPerSku: parseInt(e.target.value) || 1,
                          }))
                        }
                        onKeyDown={(e) => {
                          if (e.key === "Enter") confirmAddSku();
                    ...
                    <td className="px-2 py-2 text-center border-b border-r border-gray-200">
                      <input
                        ...
                        onChange={(e) =>
                          setNewSku((s) => ({
                            ...s,
                            generatePickingLabel: e.target.checked,
                          }))
                        }
                ...
                {skus.map((sku) => {
                  ...
                  return (
                    <tr
                      key={sku.id}
                      className={`${
                        isEditing ? "bg-blue-50/30" : "hover:bg-muted/30"
                      }`}
                    >
                      <td className="px-2 py-2 text-center border-b border-r border-gray-200 relative">
                        {isEditing ? (
                          ...
                          <button
                            ...
                            onClick={() =>
                              setOpenMenuSkuId(
                                openMenuSkuId === sku.id ? null : sku.id
                              )
                            }
                        ...
                        {openMenuSkuId === sku.id && !isEditing && (
                          <div className="absolute top-0 left-1 -translate-y-full -mt-2 z-20 w-56 bg-white border rounded shadow-lg text-sm py-1 text-left">
                            <button
                              type="button"
                              onClick={() => startEditSku(sku.id)}
                              className="block w-full text-left px-3 py-1.5 hover:bg-muted/40"
                            >
                              Editar
                            </button>
                            <button
                              type="button"
                              onClick={() =>
                                openSheetForSku(sku.id, "barcodes")
                              }
                              className="block w-full text-left px-3 py-1.5 hover:bg-muted/40"
                            >
                              Código de barras e Dimensões
                            </button>
                      ...
                      <td className="px-2 py-2 border-b border-r border-gray-200">
                        {isEditing ? (
                          <input
                            ...
                            className="h-8 w-full border rounded px-2 text-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#0c9abe]"
                          />
                        ) : (
                          sku.description
                        )}
                      ...
                      <td className="px-2 py-2 border-b border-r border-gray-200">
                        {isEditing ? (
                          <input
                            ...
                            className="h-8 w-24 border rounded px-2 text-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#0c9abe]"
                          />
                        ) : (
                          sku.unitsPerSku
                        )}
                      ...
                      <td className="px-2 py-2 text-center border-b border-r border-gray-200">
                        {isEditing ? (
                          <input
                            type="checkbox"
                            checked={editDraft?.fractional || false}
                            onChange={(e) =>
                              setEditDraft((d) =>
                                d ? { ...d, fractional: e.target.checked } : d
                              )
                            }
                          />
                        ) : sku.fractional ? (
                      ...
                      <td className="px-2 py-2 text-center border-b border-r border-gray-200">
                        {isEditing ? (
                          <input
                            type="checkbox"
                            checked={editDraft?.generatePickingLabel || false}
                            onChange={(e) =>
                              setEditDraft((d) =>
                                d
                                  ? {
                ...
                {!addingSku && skus.length === 0 && (
                  <tr>
                    <td
                      colSpan={5}
                      className="px-3 py-8 text-center text-sm text-muted-foreground border-b border-gray-200"
                    >
                ...
                <div className="flex flex-col gap-4">
                  <div className="flex gap-2 items-end">
                    <div className="flex-1">
                      <label className="text-xs font-medium text-[#334b52]">
                      ...
                      <input
                        ...
                        className="h-9 w-full border rounded px-2 text-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#0c9abe]"
                      />
                    </div>
                    <button
                      type="button"
                      onClick={addBarcode}
                      className="h-9 px-4 rounded bg-[#008bb1] text-white text-xs font-medium hover:bg-[#007697]"
                    >
                      Adicionar
                    </button>
                  </div>
                  <div className="border rounded divide-y">
                    ...
                    {skus.find((s) => s.id === openSkuSheetId)?.barcodes
                      .length === 0 && (
                      <div className="px-3 py-6 text-center text-xs text-muted-foreground">
                        Nenhum código adicionado
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    return (
                      <div className="flex flex-col gap-1" key={key}>
                        ...
                        <input
                          ...
                          className="h-9 w-full border rounded px-2 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#0c9abe]"
                        />
                      </div>
                    );
                  })}
                  <div className="flex flex-col gap-1 col-span-2">
                    ...
                    <input
                      disabled
                      value={(
                        skus.find((s) => s.id === openSkuSheetId)?.dimensions
                          ?.volumeM3 ?? 0
                      ).toFixed(4)}
                      className="h-9 w-full border rounded px-2 bg-muted/40 text-muted-foreground"
                    />
                  </div>
                </div>
              )}
              ...
              <div className="flex gap-2 justify-end">
                <SheetClose asChild>
                  <button
                    type="button"
                    onClick={closeSheet}
                    className="h-9 px-5 rounded border text-sm font-medium hover:bg-muted/40"
                  >
                    Fechar
                  </button>
                </SheetClose>
