...
import { useUpdateCarrier } from '@/lib/hooks/use-update-carrier'
import { useDeleteCarrier } from '@/lib/hooks/use-delete-carrier'
...
export default function TransportadoraPage() {
  ...
  const { data, refetch, isFetching } = useCarriers({ q, status, page, limit: pageSize, sort: 'createdAt', order: 'asc' })
    ...
    <div className="p-6 space-y-4">
      <Breadcrumb className="mb-2">
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage>Cadastro de transportadora</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>

      <h1 className="text-2xl font-semibold">Transportadora</h1>

      <div className="border rounded-md p-3">
        <div className="text-sm font-medium mb-2">Transportadoras cadastradas</div>
        {status !== 'TODOS' && (
        ...
        <div className="flex items-center gap-3">
          <Link to="/settings/transportadora/new" className="px-3 py-2 rounded-md text-white" style={{ backgroundColor: '#2f8ac9' }}>+ Nova transportadora</Link>
      ...
      <div className="border rounded-md overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {items.map((car) => (
              <Row key={car.id} car={car} onChanged={() => { setPage(1); refetch() }} />
            ))}
          </tbody>
        </table>
      </div>
...
function Row({ car, onChanged }: { car: Carrier; onChanged: () => void }) {
  const toastApi = useToast()
  const updateMut = useUpdateCarrier()
  const deleteMut = useDeleteCarrier()
  const [confirmOpen, setConfirmOpen] = useState(false)

  const toggleAtivo = async () => {
    try {
      await updateMut.mutateAsync({ id: car.id, data: { active: car.situacao !== 'ATIVO' } })
  ...
  const onDelete = async () => {
    try {
      await deleteMut.mutateAsync(car.id)
      toastApi.show({ message: 'Transportadora excluída', kind: 'success' })
      onChanged()
    } catch (e) {
      ...
      <td className="px-3 py-2">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start">
            <DropdownMenuItem onClick={toggleAtivo}>{car.situacao === 'ATIVO' ? 'Inativar' : 'Ativar'}</DropdownMenuItem>
        ...
        <ConfirmationDialog open={confirmOpen} title="Confirmação" description={`Deseja excluir a transportadora “${car.nome}”?`} confirmText="Excluir" onConfirm={onDelete} onCancel={() => setConfirmOpen(false)} />
      </td>
      <td className="px-3 py-2">{car.documento}</td>
      <td className="px-3 py-2">{car.nome}</td>
      <td className="px-3 py-2">{car.uf}</td>
      <td className="px-3 py-2">{car.inscricaoEstadual ?? 'Não informada'}</td>
      <td className="px-3 py-2"><span className={`px-2 py-1 rounded text-xs ${car.situacao === 'ATIVO' ? 'bg-green-100 text-green-800' : 'bg-gray-200 text-gray-700'}`}>{car.situacao}</span></td>
